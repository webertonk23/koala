<?php
	...
	class Import{
		...
		private function GetLayout($Id_Lay){
			$Read = new Read;
			$Read->ExeRead('Layout WITH (NOLOCK)', "WHERE Id_Lay = :Id", "Id={$Id_Lay}");
		...
		public function ProcImport(){
			if(!empty($this->Dados) AND is_array($this->Dados)){	
				foreach($this->Dados as $Chave => $Valores){
					...
					// Debug($Registros);
					// Debug($this->ValidaDados($Registros));
		...
		private function IncPes($Dados = array()){
			...
			$Read->ExeRead("Pessoas WITH (NOLOCK)", "WHERE cpfcnpj_pes = :cpfcnpj_pes", "cpfcnpj_pes=".$Check->CpfCnpj($Dados['Pessoal']['cpfcnpj_pes']));
			$Pessoa = ($Read->GetRowCount() > 0) ? $Read->GetResult()[0]['Id_Pes'] : null;
			
			if(!empty($Pessoa)){
				if(!empty($Dados['Pessoal']['sexo_pes'])){
					$Update->ExeUpdate("Pessoas", array('sexo_pes' => $Dados['Pessoal']['sexo_pes']), "WHERE id_pes = :id", "id={$Pessoa}");
				}
				...
				$Read->ExeRead("Fichas WITH (NOLOCK)", "WHERE idPes_ficha = :idPes_ficha AND contrato_ficha = :Contrato AND IdCart_ficha = :IdCart_ficha", "idPes_ficha={$Pessoa}&Contrato={$Dados['Fichas']['contrato_ficha']}&IdCart_ficha={$Dados['Fichas']['IdCart_ficha']}");
				
				if($Read->GetRowCount() == 0){
					$Create->ExeCreate('Fichas', $Dados['Fichas']);
				}else{
					$Fichas['dtatu_ficha'] = date("Y-m-d");
					
					if(!empty($Dados['Fichas']['codesp_ficha'])){
						$Fichas["codesp_ficha"] =  (int) $Dados['Fichas']["codesp_ficha"];
					}
					
					if(!empty($Dados['Fichas']['especie_ficha'])){
						$Fichas["especie_ficha"] = $Dados['Fichas']["especie_ficha"];
					}
					
					if(!empty($Dados['Fichas']['cc_ficha'])){
						$Fichas["cc_ficha"] = $Dados['Fichas']["cc_ficha"];
					}
					
					if(!empty($Dados['Fichas']['banco_ficha'])){
						$Fichas["banco_ficha"] = $Dados['Fichas']["banco_ficha"];
					}
					
					if(!empty($Dados['Fichas']['salario_ficha'])){
						$Fichas["salario_ficha"] = (float) str_replace(",", ".", $Dados['Fichas']["salario_ficha"]);
					}
					
					if(!empty($Dados['Fichas']['limite_ficha'])){
						$Fichas["limite_ficha"] = (float) str_replace(",", ".", $Dados['Fichas']["limite_ficha"]);
					}
					
					if(!empty($Dados['Fichas']['limitesaque_ficha'])){
						$Fichas["limitesaque_ficha"] = (float) str_replace(",", ".", $Dados['Fichas']["limitesaque_ficha"]);
					}
					
					if(!empty($Dados['Fichas']['margem_ficha'])){
						$Fichas["margem_ficha"] = (float) str_replace(",", ".", $Dados['Fichas']["margem_ficha"]);
					}
					
					if(!empty($Dados['Fichas']['qtdemp_ficha'])){
						$Fichas["qtdemp_ficha"] = $Dados['Fichas']["qtdemp_ficha"];
					}
					
					if(!empty($Dados['Fichas']['desent_ficha'])){
						$Fichas["desent_ficha"] = $Dados['Fichas']["desent_ficha"];
					}
					
					if(!empty($Dados['Fichas']['ent_ficha'])){
						$Fichas['ent_ficha'] = (int) $Dados['Fichas']['ent_ficha'];
					}
					
					if(!empty($Dados['Fichas']['convenio_ficha'])){
						$Fichas['convenio_ficha'] = trim($Dados['Fichas']['convenio_ficha']);
					}
					
					if(!empty($Fichas)){
						$Update->ExeUpdate("Fichas", $Fichas, "WHERE id_ficha = :id", "id={$Read->GetResult()[0]['Id_Ficha']}");
					}
				}
				...
				if(!empty($Dados['Telefones'])){
					foreach($Dados['Telefones'] as $T){
						$Read->ExeRead("Telefones WITH (NOLOCK)", "WHERE idPes_tel = :IdPes AND CONCAT(ddd_tel, telefone_tel) = :Telefone", "IdPes={$Pessoa}&Telefone={$T['telefone_tel']}");
		...
		private function ValidaDados($Dados){
			...
			if(!empty($Dados['Pessoal']['cidade_pes'])){
				$Dados['Pessoal']['cidade_pes'] = utf8_encode($Check->Name($Dados['Pessoal']['cidade_pes']));
			...
			if(!empty($Dados['Fichas']['somaparc_ficha'])){
				$Dados['Fichas']['somaparc_ficha'] = (float) $Dados['Fichas']['somaparc_ficha'];
			...
			if(!empty($Dados['Fichas']['vldesc_ficha'])){
				$Dados['Fichas']['vldesc_ficha'] = (float) str_replace(",", ".", str_replace(".", "", $Dados['Fichas']['vldesc_ficha']));
			}else if(isset($Dados['Fichas']['vldesc_ficha'])){
				unset($Dados['Fichas']['vldesc_ficha']);
			}
			
			if(!empty($Dados['Fichas']['vlmargemcomp_ficha'])){
				$Dados['Fichas']['vlmargemcomp_ficha'] = (float) str_replace(",", ".", str_replace(".", "", $Dados['Fichas']['vlmargemcomp_ficha']));
			}else if(isset($Dados['Fichas']['vlmargemcomp_ficha'])){
				unset($Dados['Fichas']['vlmargemcomp_ficha']);
			}

			if(!empty($Dados['Fichas']['vllibmargemcomp_ficha'])){
				$Dados['Fichas']['vllibmargemcomp_ficha'] = (float) str_replace(",", ".", str_replace(".", "", $Dados['Fichas']['vllibmargemcomp_ficha']));
			}else if(isset($Dados['Fichas']['vllibmargemcomp_ficha'])){
				unset($Dados['Fichas']['vllibmargemcomp_ficha']);
			}
			
			if(!empty($Dados['Fichas']['venc_ficha'])){
				$Dados['Fichas']['venc_ficha'] = $Check->DataI($Dados['Fichas']['venc_ficha']);
			}else if(isset($Dados['Fichas']['venc_ficha'])){
				unset($Dados['Fichas']['venc_ficha']);
			}
			
			if(!empty($Dados['Fichas']['valorpmt_ficha'])){
				$Dados['Fichas']['valorpmt_ficha'] = (float) str_replace(",", ".", str_replace(".", "", $Dados['Fichas']['valorpmt_ficha']));
			}else if(isset($Dados['Fichas']['valorpmt_ficha'])){
				unset($Dados['Fichas']['valorpmt_ficha']);
			}

			if(!empty($Dados['Fichas']['valorprincipal_ficha'])){
				$Dados['Fichas']['valorprincipal_ficha'] = (float) str_replace(",", ".", str_replace(".", "", $Dados['Fichas']['valorprincipal_ficha']));
			}else if(isset($Dados['Fichas']['valorprincipal_ficha'])){
				unset($Dados['Fichas']['valorprincipal_ficha']);
			}
			
			if(!empty($Dados['Fichas']['primeirovcto_ficha'])){
				$Dados['Fichas']['primeirovcto_ficha'] = $Check->DataI($Dados['Fichas']['primeirovcto_ficha']);
			}else if(isset($Dados['Fichas']['primeirovcto_ficha'])){
				unset($Dados['Fichas']['primeirovcto_ficha']);
			}

			if(!empty($Dados['Fichas']['convenio_ficha'])){
				$Dados['Fichas']['convenio_ficha'] = trim($Dados['Fichas']['convenio_ficha']);
			}else if(isset($Dados['Fichas']['convenio_ficha'])){
				unset($Dados['Fichas']['convenio_ficha']);
			}
